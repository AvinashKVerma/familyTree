...
app.post("/users", async (req, res) => {
  ...
  try {
    ...
    const values = [uniqueId, name, img, gender, mid, fid, title, spouse, display];
...
app.put("/spouse/:id", async (req, res) => {
  ...
  try {
    ...
    const personResult = await pool.query("SELECT spouse FROM familytree WHERE id = $1", [id]);

    let spouses = personResult.rows[0].spouse || [];

    spouses = spouses.filter(Boolean).map(String);

    if (!spouses.includes(String(newSpouseId))) {
      spouses.push(String(newSpouseId));
    }

    await pool.query("UPDATE familytree SET spouse = $2 WHERE id = $1", [id, spouses]);

    // Update new spouse
    const spouseResult = await pool.query("SELECT spouse FROM familytree WHERE id = $1", [newSpouseId]);

    let spouseList = spouseResult.rows[0].spouse || [];

    spouseList = spouseList.filter(Boolean);

    if (!spouseList.includes(id)) {
      spouseList.push(id);
    }

    await pool.query("UPDATE familytree SET spouse = $2 WHERE id = $1", [newSpouseId, spouseList]);

    res.status(200).json({
      message: "Spouse relationship added successfully",
    });
  } catch (error) {
    console.error(error);
    res.status(500).json({
