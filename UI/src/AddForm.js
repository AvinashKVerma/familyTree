...
const AddForm = (props) => {
  const { selectedItem, handleSubmit, setFormSwap, formSwap, addElement, selectedOption, fetchData } = props;
  ...
  const [spouse, setSpouse] = useState(selectedItem.spouse ? selectedItem.spouse : []);
  const [children, setChildren] = useState(selectedItem.children ? selectedItem.children : []);
  ...
  const handleNewNode = () => {
    if (selectedOption === "Child") {
      const validSpouse = selectedItem.spouse?.find((spouse) => spouse && spouse.display);

      if (selectedItem.gender === "male") {
        setNewFId(selectedItem.id);

        if (validSpouse) {
          setNewMId(validSpouse.id);
        }
      } else {
        setNewMId(selectedItem.id);

        if (validSpouse) {
          setNewFId(validSpouse.id);
        }
      }
    } else {
      const newData = {
